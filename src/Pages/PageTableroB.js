...
function PageTableroB() {
  ...
  useEffect(() => {
    const nada = onSnapshot(doc(db, "Jugadores", "SegundoJugador"), (doc) => {
      console.log("Current data: ", doc.data());
      getTablas();
    });
    const nope = onSnapshot(
      doc(db, "Jugadores", "AtaqueSegundoJugador"),
      (doc) => {
        console.log("Current data: ", doc.data());
        getTablas();
      }
    );
    const fall = onSnapshot(doc(db, "Jugadores", "PrimerJugador"), (doc) => {
    ...
    if (gano === true) {
      ...
      return (
        <div
          ...
          style={{ height: "100vh" }}
        >
          <Tablero table={myOcean} key={"tab1B"} />
          <Tablero table={myAtacks} key={"tab2B"} />
          <FormAtaque
            ataque={myAtacks}
            impacto={hisTable}
            user={"AtaqueSegundoJugador"}
            enemy={"PrimerJugador"}
